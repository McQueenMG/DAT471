...
def linear_scan(X, Q, b = None):
    ...
    if b is None:
        ...
        # Faster matrix multiplication version with numpy
        if b > m:
            b = m
        index = b
        while index <= m:
            # compute sums of squares for the batch and the dataset
            Q_batch = Q[index-b:index]
            Q_squares = np.sum(Q_batch**2, axis=1)[:, np.newaxis]
            X_squares = np.sum(X**2, axis=1)[np.newaxis, :]
            # compute the squared Euclidean distances using the formula
            # ||q-x||^2 = ||q||^2 + ||x||^2 - 2*q.x
            D_squared = Q_squares + X_squares - 2 * Q_batch.dot(np.transpose(X))
            I[index-b:index] = np.argmin(D_squared, axis=1)
            index += b
        remains = m % b
        index -= b
        if remains > 0:
            Q_remains = Q[index:index+remains]
            Q_squares = np.sum(Q_remains**2, axis=1)[:, np.newaxis]
            X_squares = np.sum(X**2, axis=1)[np.newaxis, :]
            D_squared = Q_squares + X_squares - 2 * Q_remains.dot(np.transpose(X))
            I[index:index+remains] = np.argmin(D_squared, axis=1)
    return I
