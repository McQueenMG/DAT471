...
FILENAME_PATTERN = re.compile(
	r"^assignment7prob3-(?P<database>.+)-(?P<batch_size>\d+)-(?P<query_size>tiny|small|medium|big)\.out$"
)
...
def collect_results(output_dir: Path):
	...
	for path in sorted(output_dir.glob("assignment7prob3-*.out")):
...
def main() -> None:
	...
	parser = argparse.ArgumentParser(
		description=(
			...
			"from Assignment 7 Problem 3 output files."
		)
	)
	parser.add_argument(
		"--output-dir",
		...
		help="Directory containing assignment7prob3-*.out files (default: ./output)",
	)
