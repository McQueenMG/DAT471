...
def linear_scan(X, Q, b = None):
    ...
    if b is None:
        ...
        # Faster batched version with numpy
        index = b
        while index <= m:
            Q_batch = Q[index-b:index] 
            D = Q_batch[:, np.newaxis, :] - X[np.newaxis, :, :]
            EUdist = np.linalg.norm(D, axis=2)
            I[index-b:index] = np.argmin(EUdist, axis=1)
            index += b
        remains = m % b
        index -= b
        if remains > 0:
            Q_remains = Q[index:index+remains]
            D = Q_remains[:, np.newaxis, :] - X[np.newaxis, :, :]
            EUdist = np.linalg.norm(D, axis=2)
            I[index:index+remains] = np.argmin(EUdist, axis=1)
