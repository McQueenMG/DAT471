...
FILENAME_PATTERN = re.compile(
	r"^assignment7prob5-(?P<database>.+)-(?P<batch_size>\d+)-(?P<query_size>tiny|small|medium|big)\.out$"
)
...
def collect_results(output_dir: Path):
	...
	for path in sorted(output_dir.glob("assignment7prob5-*.out")):
...
def main() -> None:
	...
	parser.add_argument(
		...
		help="Directory containing assignment7prob5-*.out files (default: ./output)",
	)
