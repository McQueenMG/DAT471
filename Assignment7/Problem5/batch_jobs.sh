...
template_script="${script_dir}/assignment7_problem5.sh"
...
for query_size in "${query_sizes[@]}"; do
	...
	for database in pubs glove6b glove840b; do
		...
		for batch_size in "${batch_sizes[@]}"; do
			...
			temp_script=$(mktemp "${script_dir}/.assignment7_problem5.${database}.${query_size}.${batch_size}.XXXXXX.sh")
			...
			sed \
				...
				-e "s|^#SBATCH -J .*|#SBATCH -J KNN-GPU-${database}-${query_size}-b${batch_size}|" \
